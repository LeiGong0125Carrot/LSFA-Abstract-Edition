...
parser.add_argument("--w2v_model", default="/scratch/nkw3mr/ICD-SDoH-Classification/LSFA/data/SdohICD10/glove.840B.300d.gensim", type=str,
...
def convert_to_binary(text_file, label_file=None, max_len=None, vocab=None, pad='<PAD>', unknown='<UNK>'):
    ...
    '''def pad_or_truncate(sequence, max_len, pad_id):
        if len(sequence) < max_len:
            sequence.extend([pad_id] * (max_len - len(sequence)))  # Padding
        else:
    ...
    ''' with open(text_file) as fp:
        texts = []
        for line in tqdm(fp, desc='Converting token to id', leave=False):
            token_ids = [vocab.get(word, unknown) for word in line.split()]
            token_ids = pad_or_truncate(token_ids, max_len, pad)
    ...
    if label_file is not None:
        with open(label_file) as fp:
            labels = []
            for line in tqdm(fp, desc='Converting labels', leave=False):
                label_ids = [label for label in line.split()]
                label_ids = pad_or_truncate(label_ids, max_len, pad)
                labels.append(label_ids)
            labels = np.asarray(labels)'''
    texts = truncate_text(texts, max_len, vocab[pad], vocab[unknown])
